                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            if (data.ruta_xml) {
                                let urlGetXml =
                                    "{{ route('ventas.resumenes.getXml', ['resumen_id' => ':resumen_id']) }}";
                            ...
                            if (data.ruta_cdr) {
                                let urlGetCdr =
                                    "{{ route('ventas.resumenes.getCdr', ['resumen_id' => ':resumen_id']) }}";
                        ...
                        render: function(data, type, row) {
                            ...
                            if (data.send_sunat == 1) {
                                if (data.code_estado == 98 || (data.ticket && !data.code_estado)) {
                                    html +=
                                        `<button type="button" data-resumen-id="${data.id}" class="btn btn-primary btn-consultar-resumen">CONSULTAR</button>`;
                                }
                            }

                            if (data.send_sunat == 0 && !data.ticket) {
                                html +=
                                    `<button type="button" data-resumen-id="${data.id}" class="btn btn-primary btn-reenviar-resumen">REENVIAR</button>`;
                            }

                            html +=
                                `<i class="fas fa-eye btn btn-success d-flex align-items-center btn-detalle-resumen" data-resumen-id="${data.id}"></i>`;

    ...
    <div class="modal-dialog">
        <div class="modal-content animated bounceInRight">
            ...
            <div class="modal-body">
                @include('utils.modals.mdl_model.forms.form_create')
            </div>
        </div>
    </div>
</div>
@push('scripts')
    <script>
        const configMdlModel = {
            modelSelect: null
        }

        function loadMdlModel(config) {
            configMdlModel.modelSelect = config.modelSelect;
            eventsMdlCreateModelo();
        }

        function openMdlModelo() {
        ...
        async function storeModelo(formCreateModelo) {
            ...
            const nombre = document.querySelector('#descripcion_model').value || '';
                    ...
                    try {
                        ...
                        if (error.response) {
                            ...
                            if (error.response.status === 422) {
                                ...
                                pintarErroresValidacion(errors, 'model_error');
        ...
        function addModel(instance) {
            const item = {
                id: instance.id,
                text: instance.descripcion
            };
            configMdlModel.modelSelect.addOption(item);
            configMdlModel.modelSelect.clear(true);
            configMdlModel.modelSelect.refreshOptions(false);
            configMdlModel.modelSelect.addItem(item.id)
        }
    </script>
@endpush

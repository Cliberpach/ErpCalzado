...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    pintarDetalle(@json($venta_detalle));
...
function cambiarModalidadTraslado(modalidad){
    ...
    function registrarGuia(formRegistrarGuia){
        ...
        title: "Deseas generar la Guía de Remisión para la venta?",
        ...
        if (result.isConfirmed) {
            ...
            try {
                ...
                formData.append('venta_id',@json($venta->id));
                formData.append('almacen',@json($almacen_origen->id));
                formData.append('motivo_traslado',"01");
                formData.append('cliente',@json($venta->cliente_id));

                const res       =   await axios.post(route('ventas.documento.guiaStore'),formData);
